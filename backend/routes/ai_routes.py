...
async def ai_evaluate_submission(roll_number: str):
    ...
    with open("c:/Users/This PC/OneDrive/Desktop/Final/debug.log", "a", encoding="utf-8") as f:
        f.write(f"Evaluating {roll_number}\n")
    ...
    try:
        ...
        print(f"📄 Extracting student answers from: {student_pdf}")
        ...
        print(f"📚 Extracting model answers from: {key_pdf}")
        model_answers = extract_question_paper(str(key_pdf))
        
        # Step 4: Run evaluation
        print("🤖 Running AI evaluation...")
        print(f"Model keys: {model_answers.keys() if model_answers else 'None'}")
        print(f"Student keys: {student_answers.keys() if student_answers else 'None'}")
        ...
        with open("c:/Users/This PC/OneDrive/Desktop/Final/debug.log", "a", encoding="utf-8") as f:
            f.write(f"Report type: {type(evaluation_report)}\n")
            f.write(f"Report content: {str(evaluation_report)[:1000]}...\n")
